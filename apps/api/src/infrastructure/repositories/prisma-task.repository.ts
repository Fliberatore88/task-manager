...
export class PrismaTaskRepository implements ITaskRepository {
  ...
  private async findAllSortedByPriority(filters: TaskFilterDto): Promise<TaskEntity[]> {
    const conditions: Prisma.Sql[] = [];
    if (filters.status)   conditions.push(Prisma.sql`status = ${filters.status}`);
    if (filters.priority) conditions.push(Prisma.sql`priority = ${filters.priority}`);
    if (filters.assignee) conditions.push(Prisma.sql`assignee LIKE ${'%' + filters.assignee + '%'}`);
